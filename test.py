...
def evaluate_model_file(model_path, weights_path, img_path, class_names, args, batch_size=8, img_size=416,
                        n_cpu=8, iou_thres=0.5, conf_thres=0.3, nms_thres=0.15, verbose=True):
    ...
    # 模型可视化
    # x = torch.randn(8, 3, 416, 416).cuda().requires_grad_(True)
    # model.eval()
    # y = model(x)
    #
    # pic_1 = make_dot(y[2], params=dict(list(model.named_parameters()) + [('x', x)]))
    # pic_1.format = "png"
    # pic_1.directory = "pic"
    # pic_1.view()

    # 模型可视化2
    # net_str = summary(model, input_size=(3, 416, 416))
    ...
    print('FPS:', len(dataloader.dataset)/(end-start))
...
def print_eval_stats(metrics_output, class_names, verbose):
    if metrics_output is not None:
        ...
        if verbose:
            ...
            ap_table = [["Index", "Class", "precision", "recall", "AP", "f1"]]
            for i, c in enumerate(ap_class):
                # ap_table += [[c, class_names[c], "%.5f" % AP[i]]]
                ap_table += [[c, class_names[c],
                              "%.5f" % precision[i],
                              "%.5f" % recall[i],
                              "%.5f" % AP[i],
                              "%.5f" % f1[i]]]
...
def _evaluate(model, dataloader, class_names, img_size, iou_thres, conf_thres, nms_thres, verbose):
    ...
    IOUs = []
    for img_files, imgs, targets in tqdm.tqdm(dataloader, desc="Validating"):
        ...
        # 这里返回的应该是一张图片上的各类指标
        sample_metrics_t, IOU = get_batch_statistics(outputs, targets, iou_threshold=iou_thres)
        sample_metrics += sample_metrics_t
        IOUs.append(IOU)
    ...
    metrics_output = list(metrics_output)
    metrics_output.append(np.mean(IOUs))
...
def run():
    ...
    precision, recall, AP, f1, ap_class, mIOU = evaluate_model_file(
        args.model,
        args.weights,
        valid_path,
        class_names,
