...
from .utils import to_cpu, is_parallel, xywh2xyxy
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        # Define criteria
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([1.0], device=device))    # cls_pw 1.0
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([1.0], device=device))    # obj_pw 1.0
        ...
        det = model.module.module_list[-1][0] if is_parallel(model) else model.module_list[-1][0]  # Detect() module
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7 list[4.0, 1.0, 0.4]
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.sort_obj_iou:
                    ...
                    b_1, a_1, gj_1, gi_1, score_iou_center = b_1[sort_id_1], a_1[sort_id_1], gj_1[sort_id_1], gi_1[
                        sort_id_1], score_iou_center[sort_id_1]
    ...
    def build_targets(self, p, targets):
        ...
        ai = torch.arange(num_nobox, device=targets.device).float().view(num_nobox, 1).repeat(1,
        ...
        for i in range(self.nl):
            ...
            indices_center.append((b, l, gj.clamp_(0, nG0 - 1), gi.clamp_(0, nG1 - 1)))
